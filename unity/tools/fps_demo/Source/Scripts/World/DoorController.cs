...
public class DoorController : MonoBehaviour, IInteractable, ISaveableEntity
{
    ...
    [SerializeField] private string requiredKey = string.Empty;
    ...
    public void Configure(string id, Transform leaf, string keyName)
    {
        saveId = id;
        doorLeaf = leaf;
        requiredKey = keyName;
    ...
    {
        if (!string.IsNullOrEmpty(requiredKey) && !player.Inventory.HasKey(requiredKey))
        {
            return $"{requiredKey} security door locked";
        }
    ...
    public void Interact(PlayerController player)
    {
        if (!string.IsNullOrEmpty(requiredKey) && !player.Inventory.HasKey(requiredKey))
        {
            GameManager.Instance?.NotifyStatus($"{requiredKey} key required");
            return;
        }
