...
public={'complete':True,'fallback_used':False,'asset_engine':manifest['asset_engine'],'scene_name':manifest['scene_plan']['scene_name'],'gameplay_action_count':manifest['gameplay_action_count'],'build_id':'your-mom-stableanimator-scene-v5','scene_review':{'recognizability':recognizability,'defects':defects,'screenshot_sha256':verification['screenshot_sha256'],'evidence_sha256':hashlib.sha256((evidence_root/'verification.json').read_bytes()).hexdigest()},'assets':{}}
for asset_id,asset in manifest['assets'].items():
    ...
    item={'gif':'generated_assets/'+gif_target.name,'png':'generated_assets/'+png_target.name,'sheet':'generated_assets/'+sheet_target.name,'frames':asset['frame_count'],'engine':asset.get('engine'),'pose_driven':bool(asset.get('pose_driven',False)),'fallback_used':bool(asset.get('fallback_used',False))}
    if asset_id=='player':
        item['clips']={}
        for clip_name,clip in asset['clips'].items():
            ...
            item['clips'][clip_name]={'gif':str(targets['gif'].relative_to('web')),'png':str(targets['png'].relative_to('web')),'sheet':str(targets['sheet'].relative_to('web')),'frames':clip['frame_count'],'engine':clip['engine'],'pose_driven':clip['pose_driven'],'motion_control':clip['motion_control'],'pose_driver':clip['pose_driver'],'identity_encoder':clip['identity_encoder'],'min_reference_cosine':clip['min_reference_cosine'],'min_adjacent_identity_cosine':clip['min_adjacent_identity_cosine'],'max_foreground_coverage':clip['max_foreground_coverage'],'max_border_visible_ratio':clip['max_border_visible_ratio'],'semantic_motion':clip.get('semantic_motion',{}),'fallback_used':clip['fallback_used'],'review_pass':clip['review_pass'],'distinct_gif_frames':clip['distinct_gif_frames']}
...
PY
printf 'pose_driven_web_export=ok engine=%s time=%s\n' 'sdxl-reviewed-scene-assets+stableanimator-pose-driven-player' "$(date -Is)"
