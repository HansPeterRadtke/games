...
public={'complete':True,'fallback_used':False,'asset_engine':manifest['asset_engine'],'scene_name':manifest['scene_plan']['scene_name'],'gameplay_action_count':manifest['gameplay_action_count'],'assets':{}}
for asset_id,asset in manifest['assets'].items():
    ...
    item={'gif':'generated_assets/'+gif_target.name,'png':'generated_assets/'+png_target.name,'sheet':'generated_assets/'+sheet_target.name,'frames':asset['frame_count'],'engine':asset.get('engine'),'temporal_model':bool(asset.get('temporal_model',False)),'fallback_used':bool(asset.get('fallback_used',False))}
    if asset_id=='player':
        item['clips']={}
        for clip_name,clip in asset['clips'].items():
            ...
            item['clips'][clip_name]={'gif':str(targets['gif'].relative_to('web')),'png':str(targets['png'].relative_to('web')),'sheet':str(targets['sheet'].relative_to('web')),'frames':clip['frame_count'],'engine':clip['engine'],'temporal_model':clip['temporal_model'],'native_video_frames':clip['native_video_frames'],'fallback_used':clip['fallback_used'],'review_pass':clip['review_pass'],'distinct_gif_frames':clip['distinct_gif_frames']}
...
PY
printf 'temporal_web_export=ok engine=%s time=%s\n' 'sdxl-reviewed-canonical+ltx-video-temporal+birefnet-matting' "$(date -Is)"
