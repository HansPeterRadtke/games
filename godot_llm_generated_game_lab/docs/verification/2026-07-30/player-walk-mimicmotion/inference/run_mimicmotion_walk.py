...
OUT=ROOT/'raw-frames';OUT.mkdir(parents=True,exist_ok=True)
...
pose=torch.load(ROOT/'pose.pt',map_location='cpu',weights_only=True).to(dtype=torch.float16)
...
metrics={'ok':True,'engine':'Tencent MimicMotion 1.1','base':'Stable Video Diffusion XT 1.1','frames':int(video.shape[0]),'shape':list(video.shape),'load_seconds':round(infer_started-started,3),'inference_seconds':round(time.monotonic()-infer_started,3),'total_seconds':round(time.monotonic()-started,3),'seed':int(TASK.seed),'tile_size':int(TASK.num_frames),'tile_overlap':int(TASK.frames_overlap),'steps':int(TASK.num_inference_steps),'noise_aug_strength':float(TASK.noise_aug_strength),'guidance_scale':float(TASK.guidance_scale),'driver_sha256':hashlib.sha256(Path(TASK.ref_video_path).read_bytes()).hexdigest(),'reference_sha256':hashlib.sha256(Path(TASK.ref_image_path).read_bytes()).hexdigest(),'checkpoint_sha256':hashlib.sha256(Path(CFG.ckpt_path).read_bytes()).hexdigest(),'compatibility':{'architecture':platform.machine(),'python':platform.python_version(),'torch':torch.__version__,'cuda':torch.version.cuda,'diffusers':__import__('diffusers').__version__,'transformers':__import__('transformers').__version__,'video_reader':'OpenCV Decord API shim only','shim_sha256':hashlib.sha256(Path('/data/tmp/mimicmotion-compat/decord.py').read_bytes()).hexdigest()},'step_log':steps,'output':str(OUT)}
(ROOT/'inference.json').write_text(json.dumps(metrics,indent=2)+'\n')
